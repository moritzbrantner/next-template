...
export function DataStudioClient({
  ...
  const totalFields = parsedSchema.tables.reduce(
    (sum, table) => sum + table.fields.length,
    0,
  );
  const [activeTableName, setActiveTableName] = useState(
    parsedSchema.tables[0]?.name ?? '',
  );
  const activeTable =
    parsedSchema.tables.find((table) => table.name === activeTableName) ??
    parsedSchema.tables[0];
  const requiredFieldCount =
    activeTable?.fields.filter((field) => field.required).length ?? 0;
  ...
  return (
    ...
    >
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          <CardHeader>
            <CardDescription>{t('dataStudio.summary.tables')}</CardDescription>
            <CardTitle>{String(parsedSchema.tables.length)}</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader>
            <CardDescription>{t('dataStudio.summary.fields')}</CardDescription>
            <CardTitle>{String(totalFields)}</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader>
            <CardDescription>
              {t('dataStudio.summary.required')}
            </CardDescription>
            <CardTitle>{String(requiredFieldCount)}</CardTitle>
          </CardHeader>
        </Card>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[320px_minmax(0,1fr)]">
        <Card className="h-fit">
          <CardHeader>
            ...
            <CardDescription>
              {t('dataStudio.explorerDescription')}
            </CardDescription>
          </CardHeader>
          ...
          <CardHeader>
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="space-y-1">
                <CardTitle>
                  {activeTable?.label ?? t('dataStudio.emptyState')}
                </CardTitle>
                <CardDescription>
                  {activeTable?.description ?? t('dataStudio.noDescription')}
                </CardDescription>
              </div>
            ...
            <div className="grid gap-3 md:grid-cols-3">
              <div className="rounded-2xl border p-4 dark:border-zinc-800">
                <p className="text-sm text-zinc-600 dark:text-zinc-300">
                  {t('dataStudio.details.tableName')}
                </p>
                <p className="mt-2 font-medium">{activeTable?.name ?? 'N/A'}</p>
              </div>
              <div className="rounded-2xl border p-4 dark:border-zinc-800">
                <p className="text-sm text-zinc-600 dark:text-zinc-300">
                  {t('dataStudio.details.fieldCount')}
                </p>
                <p className="mt-2 font-medium">
                  {String(activeTable?.fields.length ?? 0)}
                </p>
              </div>
              <div className="rounded-2xl border p-4 dark:border-zinc-800">
                <p className="text-sm text-zinc-600 dark:text-zinc-300">
                  {t('dataStudio.details.endpoint')}
                </p>
                <p className="mt-2 font-medium">
              ...
              <div className="space-y-4">
                <div className="rounded-2xl border p-4 dark:border-zinc-800">
                  <h3 className="font-medium">
                    {t('dataStudio.fieldListTitle')}
                  </h3>
                  <div className="mt-3 space-y-3">
                    {activeTable?.fields.map((field) => (
                      <div
                        key={field.name}
                        className="rounded-xl bg-zinc-100/70 p-3 dark:bg-zinc-900"
                      >
                        <div className="flex items-center justify-between gap-3">
                          ...
                          >
                            {field.required
                              ? t('dataStudio.fieldRequired')
                              : t('dataStudio.fieldOptional')}
                          </Badge>
                        </div>
                        <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-300">
                          {field.name}
