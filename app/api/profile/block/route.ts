...
import { blockUserUseCase, unblockUserUseCase } from '@/src/domain/profile/use-cases';
...
function mapBlockProblem(code: 'NOT_FOUND' | 'VALIDATION_ERROR' | 'FORBIDDEN' | 'CONFLICT', detail: string) {
  const status = code === 'NOT_FOUND' ? 404 : code === 'FORBIDDEN' ? 403 : code === 'CONFLICT' ? 409 : 400;
  return new ProblemError(problem('/problems/profile-block', 'Unable to update block state', status, detail));
}
