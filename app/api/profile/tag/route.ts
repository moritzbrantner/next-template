...
function statusForProfileTagError(code: 'NOT_FOUND' | 'VALIDATION_ERROR' | 'FORBIDDEN' | 'CONFLICT') {
  switch (code) {
    case 'NOT_FOUND':
      return 404;
    case 'CONFLICT':
  ...
  async handler({ body, session, actorId }) {
    ...
    if (!result.ok) {
      throw new ProblemError(
        problem('/problems/profile-tag', 'Unable to update tag', statusForProfileTagError(result.error.code), result.error.message),
      );
    }
