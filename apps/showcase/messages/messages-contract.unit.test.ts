...
const expectedNamespaces = new Set(showcaseManifest.publicPages.map((page) => page.namespace));
...
describe('showcase message contract', () => {
  it('covers every public page namespace in English and German', () => {
    ...
    const missingEnglishNamespaces = Array.from(expectedNamespaces).filter((namespace) => !englishNamespaces.has(namespace));
