...
export function inferUploadKind(
  ...
  for (const [kind, extensions] of Object.entries(extensionGroups) as Array<
    [UploadKind, string[]]
  >) {
...
export function getUploadManagementHint(kind: UploadKind, sizeInBytes: number) {
  ...
  if (kind === 'video' || sizeInMegabytes >= 25) {
    return {
      ...
      detail:
        'Large files should skip your application server and upload through a presigned or resumable flow.',
    };
  }
  ...
  if (kind === 'image') {
    return {
      label: 'Preview early',
      detail:
        'Images benefit from immediate previews plus dimension checks before the transfer starts.',
    };
  }
  ...
  if (kind === 'data') {
    return {
      ...
      detail:
        'Data imports should parse headers or schema hints before they enter the persistent queue.',
    };
  }
  ...
  return {
    ...
    detail:
      'This file can usually move through the standard queue with basic type and size checks.',
  };
}
