...
export const Route = createFileRoute('/$locale/_app/admin/data-studio')({
  ...
  return (
    <AdminPageShell title={t('dataStudio.title')} description={t('dataStudio.description')}>
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          <CardHeader>
            <CardDescription>{t('dataStudio.summary.required')}</CardDescription>
              ...
              return (
                ...
                >
                  ...
                  <p
                    className={[
                      'mt-2 text-sm',
                      isActive ? 'text-zinc-100 dark:text-zinc-800' : 'text-zinc-600 dark:text-zinc-300',
                    ].join(' ')}
                  >
          ...
          <CardContent className="space-y-6">
            <div className="grid gap-3 md:grid-cols-3">
              <div className="rounded-2xl border p-4 dark:border-zinc-800">
                <p className="text-sm text-zinc-600 dark:text-zinc-300">{t('dataStudio.details.tableName')}</p>
                <p className="mt-2 font-medium">{activeTable?.name ?? 'N/A'}</p>
              </div>
              <div className="rounded-2xl border p-4 dark:border-zinc-800">
                <p className="text-sm text-zinc-600 dark:text-zinc-300">{t('dataStudio.details.fieldCount')}</p>
                <p className="mt-2 font-medium">{String(activeTable?.fields.length ?? 0)}</p>
              </div>
              <div className="rounded-2xl border p-4 dark:border-zinc-800">
                <p className="text-sm text-zinc-600 dark:text-zinc-300">{t('dataStudio.details.endpoint')}</p>
                <p className="mt-2 font-medium">/api/admin/data-studio/records</p>
              </div>
            </div>
