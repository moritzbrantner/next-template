...
type LoadAdminUserInput = {
  userId?: string;
};

const loadAdminUserDetail = createServerFn({ method: 'GET' })
  .inputValidator((input: LoadAdminUserInput | undefined) => ({
    userId: typeof input?.userId === 'string' ? input.userId : '',
  }))
  .handler(async ({ data }) => {
    return getAdminUserDetailUseCase(data.userId);
  });

export const Route = createFileRoute('/$locale/_app/admin/users/$userId')({
  beforeLoad: ({ context, params }) => {
    requireAdminPageAccess(context.session, params.locale as AppLocale);
  },
  loader: async ({ params }) => {
    const user = await loadAdminUserDetail({
      data: {
    ...
    >
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <SummaryCard
          ...
          value={formatDateTime(user.lastActivityAt, locale, t('users.lastActivityFallback'))}
        />
            ...
            <CardContent className="grid gap-6 lg:grid-cols-[auto_minmax(0,1fr)]">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <DetailField
                  ...
                  value={user.emailVerifiedAt ? formatDateTime(user.emailVerifiedAt, locale) : t('users.detail.pending')}
                />
                <DetailField
                  label={t('users.detail.fields.lockout')}
                  value={user.lockoutUntil ? formatDateTime(user.lockoutUntil, locale) : t('users.detail.none')}
                />
                      ...
                      <div className="space-y-2">
                        <div className="flex flex-wrap items-center gap-2">
                          <Badge variant={item.status === 'unread' ? 'default' : 'secondary'}>
                            {t(`users.detail.notificationStatus.${item.status}`)}
                          </Badge>
