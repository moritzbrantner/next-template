...
import { validateAndEncodeImage } from '@/src/profile/image-validation';
...
function makeJpeg(width: number, height: number) {
  return new Uint8Array([
    ...
    0xff, 0xc0, 0x00, 0x11, 0x08, (height >> 8) & 0xff, height & 0xff, (width >> 8) & 0xff, width & 0xff, 0x03, 0x01,
...
describe('validateAndEncodeImage', () => {
  ...
  it('rejects dimensions outside allowed range', async () => {
    ...
    await expect(validateAndEncodeImage(file)).rejects.toThrow('Image dimensions must be between 64x64 and 512x512.');
  });

  it('rejects unsupported mime type', async () => {
    const file = new File([new Uint8Array([0x47, 0x49, 0x46])], 'avatar.gif', { type: 'image/gif' });

    await expect(validateAndEncodeImage(file)).rejects.toThrow('Only PNG and JPEG images are supported.');
  });
});
